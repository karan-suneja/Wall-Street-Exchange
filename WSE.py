...
#Parameter Section
st.header("\N{speech balloon}Parameters")
st.markdown("")
...
if submitted2:
    ...
    def pred(user_inp):
        ...
        st.header("Actual & Predicted Data")
        form2.dataframe(test_data)
        def filedownload(df):
            csv = test_data.to_csv(index=False)
            b64 = base64.b64encode(csv.encode()).decode()
            href = f"<a href = 'data:file/csv;base64, {b64}' download = 'Prediction Data.csv'><div><span class='highlight blue' id=download style=font-weight:700;>Download Data File</span></div></a>"
            return href
        form2.markdown(filedownload(df), unsafe_allow_html=True)
...
if submitted3:
    ...
    def bssignal(com):
        ...
        df.reset_index(inplace=True)
        df['Date'] = pd.to_datetime(df['Date']).dt.date
        form3.dataframe(df[["Date", "Open", "Close", "DEMA_short", "DEMA_long", "High", "Low", "Volume"]])
        def filedownload(df):
            csv = df.to_csv(index=False)
            b64 = base64.b64encode(csv.encode()).decode()
            href = f"<a href = 'data:file/csv;base64, {b64}' download = 'Data.csv'><div><span class='highlight blue' id=download style=font-weight:700;>Download Data File</span></div></a>"
            return href
        form3.markdown(filedownload(df), unsafe_allow_html=True)
